#'Density-ratio SDM estimation with CV MAXNET
#'
#'dr_CVmaxnet is an internal function for density-ratio estimation with MAXNET
...
#' @keywords internal
dr_CVmaxnet <- function(presence_data = NULL,
  ...
  if(method == "fit"){
    ...
    #Fit model
    ratio  <- CVmaxnet::CVmaxnet(p = pa_vector,
                             data = pa_covariates,
                             f = f,
                             regmult = regmult,
                             regfun = regfun,
  ...
  if(method == "predict"){
    ...
    predict.maxnet <- getFromNamespace("predict.CVmaxnet", "CVmaxnet")
